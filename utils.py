...
def plot_tsne(config, dataset, latent, labels, labels_pred):
    ...
    show_tsne(embedding, labels_pred, 'result/%s/GMVAE-%s-%s-pred.png'%(config['data_params']['name'], 'alpha-gan', config['clustering_params']['plot']))
    ...
    if config['data_params']['labeled']:
        show_tsne(embedding, labels, 'result/%s/GMVAE-%s-%s-true.png'%(config['data_params']['name'], 'alpha-gan', config['clustering_params']['plot']), tlabels=dataset.tlabels)
