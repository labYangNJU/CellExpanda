...
def main(args, config):
    ...
    if args.mode == 'train':
        ...
        X_recon = model.get_reconstruction(dataset.X)
        np.savetxt('result/%s/recon.txt'%(config['data_params']['name']), X_recon,fmt="%.1f",delimiter = ',')
        
    
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CellExpanda,an expandable deep generative framework, to learn a holistic representation of cells from paired single-cell multiomics.')
