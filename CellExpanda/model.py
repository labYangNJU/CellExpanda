...
class GAMM(nn.Module):
    ...
    def get_latent(self, x, batch_size = 256):
    ...
    def get_reconstruction(self, x, batch_size=256):
        ...
        for i in range(batch_num):
            ...
            qz_m, qz_v, z = self.z_encoder(X_batch)
            ql_m, ql_v, library = self.l_encoder(X_batch)
            _, _, px_rate, _, _ = self.decoder(z, library)

            #x_gen = px_rate
            x_gen[i*batch_size:(i+1)*batch_size] = px_rate.detach().cpu().numpy()
        print('shape(x_gen)', x.shape, x_gen.shape)
        return x_gen
