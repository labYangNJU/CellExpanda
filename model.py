...
class GAMM(nn.Module):
    ...
    def get_latent(self, x):
        ...
        for i in range(batch_num):
            X_batch = FLOAT(x[i*batch_size:(i+1)*batch_size])
            #self.z_encoder.cuda()
    ...
    def forward(self, x, local_l_mean, local_l_var, writer=None, global_step=0):
        ...
        kl_divergence_l = kl(Normal(ql_m, torch.sqrt(ql_v)), Normal(torch.full(ql_m.shape, local_l_mean[0][0]).cuda(), torch.full(ql_v.shape, np.sqrt(local_l_var[0][0])).cuda())).sum(dim=1)     
        
        if global_step == 129:
            print(global_step, 'x', x.shape, x.mean(), px_rate.shape, px_rate.mean(), px_r.shape, px_r.mean())
        ...
        x_real = FLOAT(self.dataset.X[idx[:x.size(0)]])
