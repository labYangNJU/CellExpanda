...
def recon(args):
    ...
    model = GAATAC(args,len(peaks), index_sels=index_sels,n_batch=args.n_batch * use_batches, X=gene_dataset.X,
               n_hidden=args.n_hidden, n_latent=args.n_latent, dropout_rate=args.dropout, use_cuda=use_cuda, RNA_dim=args.RNA_dim,selective_weight=args.selective_weight)#, writer=writer)
    ...
    X_recon = model.get_reconstruction(X)
    np.savetxt('result/%s/recon.txt'%(args.dataset), X_recon,fmt="%.1f",delimiter = ',')
